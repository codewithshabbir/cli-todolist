...
async function todoSelection(getId: number) {
  ...
  while (!todoSelectionCheck) {
    ...
    if (todosSelections.todosSelect.toLowerCase() === "select todo") {
      let currentUser = getExistingUsers().find(
      ...
      todosSelections.todosSelect.toLowerCase() === "delete all todo"
    ) {
...
async function addTodo(getId: number) {
...
async function updateTodo(getId: number) {
  let currentUser = getExistingUsers().find((user) => user.userId === getId);
  ...
  if (currentUser) {
    if (!currentUser.myTodos || currentUser.myTodos.length === 0) {
      ...
      const todoToUpdate = currentUser.myTodos.find(
        (todo) => todo.todoId === updateTodoId.todoId
      );
...
async function deleteTodo(getId: number) {
  let currentUser = getExistingUsers().find((user) => user.userId === getId);
  ...
  if (currentUser) {
    if (!currentUser.myTodos || currentUser.myTodos.length === 0) {
      ...
      const todoIndexToDelete = currentUser.myTodos.findIndex(
        (todo) => todo.todoId === deleteTodoId.todoId
      );
...
async function deleteAllTodos(getId: number) {
  let currentUser = getExistingUsers().find((user) => user.userId === getId);
